...
class ProductImageModelTest(TestCase):
    def setUp(self):
        ...
        self.image = ProductImage.objects.create(
            product=self.product, alt_text="Test Image"
        )
...
class ReviewModelTest(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            phone_number="09658456633", password="12345"
        )
        self.category = Category.objects.create(
            title="Test Category", slug="test-category"
        )
        ...
        self.review = Review.objects.create(
            product=self.product, user=self.user, rating=4, comment="Test comment"
        )
    ...
    def test_review_creation(self):
        ...
        self.assertEqual(
            self.review.__str__(),
            f"Review of {self.product.title} by {self.user.phone_number}",
        )
...
class InventoryModelTest(TestCase):
    def setUp(self):
        ...
        self.inventory = Inventory.objects.create(
            product_variant=self.variant, quantity=10
        )
