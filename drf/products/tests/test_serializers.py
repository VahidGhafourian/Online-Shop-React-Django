...
from products.serializers import (
    ...
    ProductImageSerializer, TagSerializer, ReviewSerializer, InventorySerializer
)
...
class CategorySerializerTest(TestCase):
    ...
    def test_contains_expected_fields(self):
        ...
        self.assertEqual(set(data.keys()), set(['id', 'parent', 'title', 'slug', 'created_at', 'updated_at', 'product_attributes_schema', 'variant_attributes_schema']))
...
class ProductVariantSerializerTest(TestCase):
    def setUp(self):
        self.category = Category.objects.create(title='Test Category', slug='test-category')
        self.product = Product.objects.create(category=self.category, title='Test Product', slug='test-product')
    ...
    def test_contains_expected_fields(self):
        data = self.serializer.data
        self.assertEqual(set(data.keys()), set(['id', 'product', 'price', 'quantity', 'attributes']))
...
class ReviewSerializerTest(TestCase):
    def setUp(self):
        User = get_user_model()
        self.user = User.objects.create_user(phone_number='09141548965', password='12345')
        self.category = Category.objects.create(title='Test Category', slug='test-category')
        self.product = Product.objects.create(category=self.category, title='Test Product', slug='test-product')
...
class InventorySerializerTest(TestCase):
    def setUp(self):
        self.category = Category.objects.create(title='Test Category', slug='test-category')
        self.product = Product.objects.create(category=self.category, title='Test Product', slug='test-product')
        self.variant = ProductVariant.objects.create(product=self.product, title='Test Variant', slug='test-variant', price=100)
