...
class ProductSearchFilterViewTestCase(APITestCase):
    def setUp(self):
        ...
        self.user = get_user_model().objects.create_user(
            phone_number="09521478965", password="testpass123"
        )
        ...
        ProductVariant.objects.create(
            product=self.product1, title="64GB", slug="64gb", price=50000
        )
        ProductVariant.objects.create(
            product=self.product1, title="128GB", slug="128gb", price=60000
        )
        ProductVariant.objects.create(
            product=self.product2, title="Paperback", slug="paperback", price=5000
        )
        ProductVariant.objects.create(
            product=self.product2, title="Hardcover", slug="hardcover", price=7000
        )
    ...
    def test_filter_by_price_range(self):
        response = self.client.get(
            self.url, {"min_price": "6000", "max_price": "55000"}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 2)
...
class ReviewViewTest(APITestCase):
    def setUp(self):
        ...
        self.url = reverse(
            "products:review-list", kwargs={"product_id": self.product.id}
        )
        self.admin_user = User.objects.create_superuser("09010521833", "password123")
