...
class NotificationModelTest(TestCase):
    ...
    def test_notification_creation(self):
        ...
        self.assertEqual(notification.__str__(), f"{self.user.phone_number} - custom - {notification.created_at}")

    def test_notification_ordering(self):
        Notification.objects.create(user=self.user, notification_type='custom', message='Old notification')
        timezone.now()
        new_notification = Notification.objects.create(user=self.user, notification_type='custom', message='New notification')
...
class NotificationSerializerTest(TestCase):
    ...
    def test_contains_expected_fields(self):
        ...
        self.assertCountEqual(data.keys(), ['id', 'notification_type', 'message', 'is_read', 'created_at', 'content_type', 'object_id'])

    def test_notification_type_field_content(self):
        data = self.serializer.data
        self.assertEqual(data['notification_type'], self.notification_attributes['notification_type'])
...
class NotificationViewSetTest(TestCase):
    ...
    def test_retrieve_notification(self):
        response = self.client.get(reverse('notifications:notification-detail', kwargs={'pk': self.notification.id}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['message'], 'Test notification')

    def test_delete_notification(self):
        response = self.client.delete(reverse('notifications:notification-detail', kwargs={'pk': self.notification.id}))
    ...
    def test_get_by_timeframe_invalid_data(self):
        response = self.client.post(reverse('notifications:notification-get-by-timeframe'), data={})
