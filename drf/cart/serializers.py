...
class CartItemSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = CartItem
        fields = ['id', 'cart', 'product_variant', 'quantity', 'price', 'discounted_price', 'subtotal']
    ...
    def get_discounted_price(self, instance):
        ...
        ).order_by('-discount_percent')  # Get the highest discount
...
class CartSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Cart
        fields = ['id', 'items', 'total_price', 'total_discounted_price', 'coupon', 'final_price', 'status', 'created_at', 'updated_at']
        read_only_fields = ['created_at', 'updated_at']

    def to_representation(self, instance):
        representation = super().to_representation(instance)
        total_price = sum(item.product_variant.price * item.quantity for item in instance.items.all())
        total_discounted_price = sum(self.fields['items'].child.get_discounted_price(item) * item.quantity for item in instance.items.all())
        ...
        representation['final_price'] = max(final_price, 0)  # Ensure price doesn't go below 0
        return representation
