...
from products.factories import ProductVariantFactory, ProductFactory, CategoryFactory
from account.factories import UserFactory
from payments.factories import DiscountFactory, CouponFactory
...
class CartItemSerializerTest(TestCase):
    ...
    def test_contains_expected_fields(self):
        ...
        self.assertCountEqual(data.keys(), ['id', 'product_variant', 'quantity', 'price', 'discounted_price', 'subtotal'])
    ...
    def test_discounted_price_calculation(self):
        ...
        cart_item = CartItemFactory(cart=self.cart, product_variant=product_variant, quantity=2)

        # Create a discount
        discount = DiscountFactory(discount_percent=10, applicable_categories=[category])
    ...
    def test_validation_quantity_exceeds_stock(self):
        ...
        self.assertIn('Requested quantity exceeds available stock.', str(serializer.errors))
    ...
    def test_validation_product_not_available(self):
        ...
        serializer = CartItemSerializer(data=data)
        self.assertFalse(serializer.is_valid())
        self.assertIn('Requested product is not available at the moment.', str(serializer.errors))
...
class CartSerializerTest(APITestCase):
    def setUp(self):
        ...
        self.cart_item1 = CartItemFactory(cart=self.cart, product_variant=self.product_variant1, quantity=2, price=self.product_variant1.price)
        self.cart_item2 = CartItemFactory(cart=self.cart, product_variant=self.product_variant2, quantity=1, price=self.product_variant2.price)
        self.serializer = CartSerializer(instance=self.cart)

    def test_contains_expected_fields(self):
        data = self.serializer.data
        self.assertCountEqual(data.keys(), ['id', 'items', 'total_price', 'total_discounted_price', 'coupon', 'final_price', 'status', 'created_at', 'updated_at'])

    def test_items_field_contains_correct_data(self):
        self.assertEqual(len(self.serializer.data['items']), 2)
        self.assertEqual(self.serializer.data['items'][0]['product_variant'], self.cart_item1.product_variant.id)
        self.assertEqual(self.serializer.data['items'][1]['product_variant'], self.cart_item2.product_variant.id)

    def test_total_price_calculation(self):
        self.assertEqual(self.serializer.data['total_price'], 4000)  # (1000 * 2) + (2000 * 1)
