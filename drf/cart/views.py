    ...
    def post(self, request):
        ...
        try:
            cart_item = CartItem.objects.get(
                cart=cart, product_variant_id=product_variant_id
            )
...
class ApplyCouponView(APIView):
    ...
    def post(self, request):
        ...
        if cart.coupon:
            return Response(
                {"error": "A coupon is already applied to this cart"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if not any(
            item.product_variant in coupon.applicable_to.all()
            or item.product_variant.product.category
            in coupon.applicable_categories.all()
