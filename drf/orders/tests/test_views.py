...
class OrderViewTestCase(TestCase):
    def setUp(self):
        ...
        self.admin_order = OrderFactory(
            items__size=1
        )  # This order is not associated with self.user
    ...
    def test_admin_order_list_view(self):
        ...
        self.assertEqual(
            len(response.data), 2
        )  # Both orders should be visible to admin
    ...
    def test_admin_refund_order(self):
        ...
        self.assertEqual(
            order.items.first().product_variant.inventory.quantity, initial_quantity + 2
        )
